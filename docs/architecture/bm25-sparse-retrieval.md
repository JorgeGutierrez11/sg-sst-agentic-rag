...
BM25 agrega recuperación léxica al corpus SG-SST. Su responsabilidad es construir un índice sparse local desde los artefactos JSONL canónicos y exponer una consulta runtime de solo lectura con una forma compatible con los resultados actuales de recuperación, incluida la integración híbrida con Chroma.
...
El índice se persiste en `data/processed/bm25/`. En runtime, `agents/shared/bm25_retrieval.py` abre ese índice existente y ejecuta consultas `top_k`; no crea índices. El grafo LangGraph actual lo consume mediante `agents/shared/hybrid_retrieval.py` junto con Chroma.
...
| Fusión híbrida | `agents/shared/hybrid_retrieval.py` | Combinar resultados BM25 y Chroma, deduplicar por identidad documental y anotar fuentes de recuperación. |
...
`load_vector_record_batch` conserva el contrato ya usado por Chroma: IDs canónicos, texto indexable y metadata plana. Esa metadata incluye `document_id` como identidad pública canónica para child chunks y documentos de tabla. La capa BM25 transforma esos registros en un corpus ordenado para que los resultados devueltos por BM25S puedan mapearse de nuevo a documentos recuperables.
...
```python
{
    ...
    "metadata": record.metadata,  # incluye document_id
}
```
...
| No aplicar stemming, lematización, sinónimos ni expansión de consulta | La primera fase prioriza trazabilidad y recuperación literal de señales jurídicas como normas, siglas y numerales. |

La implementación actual usa `stopwords="es"` tanto al indexar como al tokenizar consultas runtime. No introduce stemming, lematización, sinónimos, query expansion, reranking ni filtros metadata-aware.
...
- Los IDs de child chunks y tablas se preservan y su metadata incluye `document_id`.
- Runtime abre un índice existente y no escribe en disco.
- La salida contiene `ids`, `documents`, `metadatas` y `scores`.
- La fusión híbrida externa usa `_retrieval_sources` para trazar si cada documento vino de Chroma, BM25 o ambos.

## Fuera de alcance

Este documento cubre el componente BM25 y su contrato para recuperación híbrida. No documenta Multi-Query, query rewriting, reranking ni evaluación comparativa de técnicas.
