...
class SourceReference:
    ...
    def from_metadata(cls, metadata: dict[str, Any]) -> "SourceReference":
        ...
        source = metadata.get("source_stem") or metadata.get("source_document_id") or "unknown source"
        return cls(
            source=str(source),
    ...
    def format(self) -> str:
        ...
        article_text = f", article {self.article}" if self.article else ""
...
def fallback_answer(context: str, references: list[str]) -> str:
    ...
    if context == "No recovered context.":
        return "Recovered evidence is insufficient to answer the question."
    return "\n".join(["Grounded draft based only on recovered context:", context, "References:", *references])
